...
class FormMedia{
    ...
    #action = "";
    #bShowDelBtn = true;
    #showBtnRadioEnact = false;
    #bShowLabelAdd = true;
    #showStatusTr = true;
    #hightLightLastUpdate = true;
    #bSetStatus = false;
    ...
    #events = []; // name: upload_success, confirmdel
    ...
    #initUI(){
        ...
        tableDom.innerHTML = `<table class="table table-bordered fs--2 table-sm mb-0">
                                    <thead class="text-900 bg-200">
                                        <tr>
                                            <th class="sort" data-sort="name">${this.#translate.file_name}</th>
                                            <th class="sort" data-sort="created_at" style="position: relative; width: 105.297px;">${this.#translate.create_date}</th> 
                                            <th class="sort" data-sort="file_owner">${this.#translate.created_by}</th>                                          
                                            <th class="sort ${this.#showStatusTr ? "" : "d-none"}" data-sort="file_owner">${this.#translate.status} <b style="color: red;">*</b></th>
                                            <th style="width: 10%"></th>
                                        </tr>
                                    </thead>
                                    <tbody class="list"></tbody>
                                </table>
                                <div class="d-flex justify-content-center mt-3 mb-1 d-none">
                                ...
                                <div class="mt-2 form-check ${this.#bShowLabelAdd == true ? "" : "d-none"}" style="margin-left: 2px;">
                                    <input class="form-check-input" id="check_finish_handle_status_${this.#containerID}" onclick="finishHandleStatus('check_finish_handle_status_${this.#containerID}', '${this.#containerID}')" type="checkbox" value="" />
                                    <label class="form-check-label" for="check_finish_handle_status_${this.#containerID}">Đã kiểm tra trạng thái file đính kèm</label>
                                </div>
                                `;
        ...
        var options = {
            ...
            pagination: true,
            item: function (doc){
                var type_file = self.#getIconFile(doc.file_name);
                return `<tr class="${doc.type}" id="item-${self.#containerID}${doc.id}">
                            <td style="text-align: left" class="name">
                                <img style="margin-left: 10px; margin-right: 5px;" src="${icon_path + type_file}" alt="file">${doc.file_name}
                            </td>
                            <td style="text-align: left;vertical-align: middle;" class="create_at">${doc.created_at}</td>
                            <td style="text-align: left;vertical-align: middle;" class="file_owner">${doc.file_owner}</td>
                            <td id="radio-group-td" style="text-align: left;vertical-align: middle;" class="check-box ${self.#showStatusTr ? "" : "d-none"}">
                                <div class="radio-group" style="display: inline-block; margin-right: 10px;">
                                    <input type="radio" data-radio="radio_option" id="radio-process-${doc.id}" name="option_media-${doc.id}" value="process-${doc.id}" ${doc.dtype != null ? doc.dtype === "PROCESS" ? "checked" : "" : "checked"}>
                                    <label for="radio-process-${doc.id}">Cần xử lý</label>
                                </div>
                                <div class="radio-group" style="display: inline-block; margin-right: 10px;">
                                    <input type="radio" data-radio="radio_option" id="radio-coordinate-${doc.id}" name="option_media-${doc.id}" value="coordinate-${doc.id}" ${doc.dtype === "COORDINATE" ? "checked" : ""}>
                                    <label for="radio-coordinate-${doc.id}">Đính kèm</label>
                                </div>
                                <div class="radio-group" style="display: inline-block; margin-right: 10px;">
                                    <input type="radio" data-radio="radio_option" id="radio-reference-${doc.id}" name="option_media-${doc.id}" value="reference-${doc.id}" ${doc.dtype === "REFERENCE" ? "checked" : ""}>
                                    <label for="radio-reference-${doc.id}">Đã xử lý</label>
                                </div>
                                
                                ${self.#showBtnRadioEnact ? `<div class="radio-group" style="display: inline-block; margin-right: 10px;">
                                                            <input type="radio" data-radio="radio_option" id="radio-enact-${doc.id}" name="option_media-${doc.id}" value="enact-${doc.id}" ${doc.dtype === "ENACT" ? "checked" : ""}>
                                                            <label for="radio-enact-${doc.id}">Ban hành</label>
                                                        </div>` : ""}
                            </td>
                            <td style="text-align: center;white-space: nowrap;vertical-align: middle;">
                                <a href="${['excel.png','doc.png','power-point.png'].includes(type_file) ? `https://view.officeapps.live.com/op/view.aspx?src=https://erp.bmtu.edu.vn/mdata/hrm/${doc.file_name}&wdOrigin=BROWSELINK` : `https://erp.bmtu.edu.vn/mdata/hrm/${doc.file_name}`}" target="_blank" class="me-3" style="text-decoration: none; cursor: pointer;" type="button" data-bs-toggle="tooltip" data-bs-placement="top" title="Xem trực tiếp" >
                                    <i class = "fas fa-eye icon "></i> 
                                </a>
                                <a download href="https://erp.bmtu.edu.vn/mdata/hrm/${doc.file_name}" target="_blank" class="me-3" style="text-decoration: none; cursor: pointer;" type="button" data-bs-toggle="tooltip" data-bs-placement="top" title="Tải file về">
                                    <i class = "fas fa-file-download icon text-primary"></i>
                                </a>
                                ${self.#bShowDelBtn ? `<a id="remove-doc" data-button-remove-doc="${doc.id}" style="text-decoration: none; cursor: pointer;" title="Xóa">
            ...
            if(id){
                ...
                for (let i = 0; i < items.length; i++) {
                    ...
                    if(data.id.toString() == id.toString()){
                        ...
                        $('#form_remove_mediafile #id_mediafile_release').val(data.file_id);
                        $('#form_remove_mediafile #id_mandocfile_release').val(data.id);
                        $('#form_remove_mediafile #id_tr_render_release').val(items[i].elm.id);
        ...
        this.#modal.className = "modal fade color_background";
        this.#modal.innerHTML +=    `<div class="modal-dialog modal-dialog-centered" role="document" style="max-width: 500px">
                                        <div class="modal-content position-relative">
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <button id="confirm-modal" onclick="btnDeleteMediafile('${this.#modal.id}')" class="btn btn-primary" type="button">${this.#translate.confirm}</button>
                                            </div>
                                        </div>
                                    </div>`;
        ...
        $(`#${this.#modal.id}`).on('hidden.bs.modal', function() {
            $('#form_remove_mediafile #id_mediafile_release').val('');
            $('#form_remove_mediafile #id_mandocfile_release').val('');
            $('#form_remove_mediafile #id_tr_render_release').val('');
        })
    }
    ...
    #onInputFile(input){
        ...
        for (var i = 0; i < inputFiles.length; i++) {
            ...
            if (inputFiles[i].name.length >= 255) {
                fileLongName += `${inputFiles[i].name}\n`
            } else {
                if(!duplicate){
        ...
        if (fileLongName != "") {
            alert(`Tập tin:\n${fileLongName}Có dộ dài tên quá dài! Vui lòng thay đổi độ dài tên tập tin dưới 255 ký tự.`);
        }
        //clear value
        input.value = null;
        this.#updateList();
    ...
    #onFiledrop(e){
        ...
        for (var i = 0; i < files_drop.length; i++) {
            ...
            if (files_drop[i].name.length >= 255) {
                fileLongName += `${files_drop[i].name}\n`
            } else {
                if(!duplicate){
    ...
    #updateList(){
        ...
        for (let i = 0; i < this.#files.length; i++) {
            ...
            let item = this.#renderSelectFiles(id,file.name,file.size,file.type);
            this.#listItems.append(item);
        }
    }
    
    #renderSelectFiles(itemID,name,size,type){
        // size calc
    ...
    */
    ...
    // Fix for update file
    tableAddItems(datas){
        // make clone current items and remove isLast tags
        let currentItems = [];
        let cloneCurrent = [];
        for (let i = 0; i < this.#table.items.length; i++) {
            let data = this.#table.items[i].values();
            data.isLast = false;
            currentItems.push(data);
        }
        cloneCurrent = JSON.parse(JSON.stringify(currentItems));
        let firstLoad = cloneCurrent.length == 0
        datas.forEach((data,index)=>{
            if (firstLoad){
                if(index == 0){
                    data.isLast = true;
                }
                cloneCurrent.push(data);
            }else{
                if(index == datas.length - 1){
                    data.isLast = true;
                }
                cloneCurrent.unshift(data);
            }
           
        });

        this.removeTableItemAll();
        for (let index = 0; index < cloneCurrent.length; index++) {
            this.#table.add(cloneCurrent[index]);
        }
        var count = 0;
        var interval = setInterval(function() {
        if (count < 3) {
            $('.new-item td#radio-group-td .radio-group label').css('color', 'red');
            setTimeout(function() {
            $('.new-item td#radio-group-td .radio-group label').css('color', 'unset');
            }, 400);
            count++;
        } else {
        ...
        if(!this.#bSetStatus){
            $(this.#container).find('.undefined input[type="radio"]').prop('disabled', true);
        }
        data_tr_remove.forEach(item=>{
            $(`#${item}`).remove();
        })
    }
     ...
     * 
     */
    showEnactRadioBtn(bShow){
        this.#showBtnRadioEnact = bShow;
    }
     ...
     */
    showStatus(bShow){
        this.#showStatusTr = bShow;
    }

    


    disabledRadioButton(bShow){
        $('tr.undefined').find('input[type="radio"]').prop('disabled', bShow);
    }
    /**
     * 
     */
    ...
    showHightlightLastUpload(bShow){
        this.#hightLightLastUpdate = bShow;
    }
    ...
    init(){
        this.#initUI();
        // Khai báo button bị disable khi chưa chọn xác nhận 
        $('form button.handle_file_btn').prop('disabled', true); 
    }

}       

function finishHandleStatus(id, idform) {
    if ($(`#${id}`).is(':checked')) {
        var formId = $(`#${idform}`).closest('form').attr('id');
        $(`#${formId} button.handle_file_btn`).prop('disabled', false); 
    } else {
        var formId = $(`#${idform}`).closest('form').attr('id');
        $(`#${formId} button.handle_file_btn`).prop('disabled', true); 
    }
}

function zipFileAfterDownload(files, name) {
        // Danh sách các URL tới các tệp bạn muốn tải xuống
        var fileUrls = files || [];
      
          // Tạo một tệp ZIP
          var zip = new JSZip();
      
          // Sử dụng fetch để tải từng tệp và thêm chúng vào tệp ZIP
          ...
          // Sau khi tải tất cả các tệp và thêm chúng vào tệp ZIP, tạo tệp ZIP và tải xuống
          Promise.all(promises)
            .then(function() {
              return zip.generateAsync({ type: 'blob' });
            })
            .then(function(content) {
              var zipFilename = `Tài liệu của ${name}.zip`; // Tên tệp ZIP
              var blob = new Blob([content], { type: 'application/zip' });
      
              // Tạo một liên kết để tải xuống tệp ZIP
