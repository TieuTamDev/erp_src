...
formMediaManfile.showDeleteButton(true);
...
formMediaMandocfile.showDeleteButton(true);
...
// đưa id media vào input media_ids của form mandocfile
    $("#form_add_document_release").append(`<input name="media_ids[]" value = "${data.id}" style="display: none"></input>`)
});
...
$("#btn_add_doc").on("click",function(){
    ...
    if(sno == ""){
        ...
        formMediaManfile.removeTableItemAll();        
        var input_radio = $('input[data-radio="radio_option"]:checked');
        $("#form_add_document_release input[name='media_ids[]']").remove();
        $("#form_add_document_release input[name='option_media[]']").remove();
        for (let i = 0; i < input_radio.length; i++) {
          var parts = input_radio[i].value.split('-');
          var value = parts[1];
          $("#form_add_document_release").append(`<input name="media_ids[]" value="${value}" style="display: none">`);
...
function clickAgainRelease(mandoc, stype, department, user) {
    ...
    $('.show_info_mandoc').html(`
        ...
        <div class="info_mandoc mt-2">
            <span>- <b>Trích yếu nội dung:</b> ${mandoc.notes}</span><br>
            <span>- <b>Số đi:</b> ${mandoc.sno}</span><br>
            <span>- <b>Loại văn bản:</b> ${stype}</span><br>
            <span>- <b>Phòng ban/Đơn vị soạn thảo:</b> ${mandoc.mdepartment}</span><br>
        </div>
