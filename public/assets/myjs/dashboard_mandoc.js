...
$("#user_handle").click(function () {
	...
	for (let i = 0; i < input_radio.length; i++) {
		...
		$("#new_mandocdhandle").append(`<input name="option_media[]" value="${
			input_radio[i].value
		}" style="display: none">`);
	}
...
$("#btn_add_doc").on("click", function () {
	...
	if (ds_ids_media == undefined) {
		if (confirm("Chưa có tập tin đính kèm, xác nhận thêm và trình lên Trưởng/Phó phòng")) {} else {
			return
		}
	}
	var notes = $("#notes").val()
...
$("#btn_update_doc").on("click", function () {
	...
	if (notes == "") {
		...
		for (let i = 0; i < input_radio.length; i++) {
			...
			$("#form_add_document_infor").append(`<input name="option_media[]" value="${
				input_radio[i].value
			}" style="display: none">`);
		}
		$("#form_add_document_infor").submit();
...
$("#btn_add_staff").on("click", function () {
	...
	var newArray = $.map(arrUserId, function (item) { // Use a regular expression to extract the number from each string
	...
	flatpickr("#dt_issued_date_dealine", {dateFormat: "d/m/Y"}).setDate(staffDeadLine);
...
$("#btn_save_staff_chosses").on("click", function () {
    ...
    <tr id="tr_id_${id_select}">
        ...
        <td class="align-middle">
          <div class="d-flex">
              <div class="">
                <input class="submit-get form-check-input-to-know" id="To_know_${id_select}" type="checkbox" data-id="${id_select}" name="srole[${id_select}]" onclick="checkClearRadioSrole('${id_select}')" value="DEBIET" ${
			srole_select.includes('DEBIET') ? 'checked' : ''
		} />
                <label class="form-check-label m-0" for="To_know_${id_select}">${DEBIET}</label>
              </div>
              <div class=" ms-3">
                <input class="submit-get radio-receive form-check-input" id="Receive_${id_select}" type="radio" data-id="${id_select}" onclick="checkDupliceSrole('${id_select}','#Receive_${id_select}')" name="srole[${id_select}]" value="XULY" ${
			srole_select.includes('XULY') ? 'checked' : ''
		} />
                <label class="form-check-label m-0" for="Receive_${id_select}">${XULY}</label>
              </div>
              <div class=" ms-3">
                <input class="submit-get form-check-input" id="PHOIHOPXL_${id_select}" type="radio" data-id="${id_select}" name="srole[${id_select}]" onclick="checkClearCheckboxSrole('${id_select}')" value="PHOIHOPXL" ${
			srole_select.includes('PHOIHOPXL') ? 'checked' : ''
		}/>
	...
	flatpickr(deadLineId, {dateFormat: "d/m/Y"}).setDate(dtDeadLine);
	...
	if (dataUsers.length > 0) {
        ...
        <tr id="tr_id_${
            ...
            <td class="align-middle">
              <div class="d-flex">
                  <div class="">
                    <input class="submit-get form-check-input-to-know" id="To_know_${
					item.users.id
				}" type="checkbox" data-id="${
					item.users.id
				}" name="srole[${
					item.users.id
				}]" onclick="checkClearRadioSrole('${
					item.users.id
				}')" value="DEBIET" checked />
                    <label class="form-check-label m-0" for="To_know_${
					item.users.id
				}">${DEBIET}</label>
                  </div>
                  <div class=" ms-3">
                    <input class="submit-get radio-receive form-check-input" id="Receive_${
					item.users.id
				}" type="radio" data-id="${
					item.users.id
				}" onclick="checkDupliceSrole('${
					item.users.id
				}','#Receive_${
					item.users.id
				}')" name="srole[${
                    ...
                    <label class="form-check-label m-0" for="Receive_${
					item.users.id
				}">${XULY}</label>
                  </div>
                  <div class=" ms-3">
                    <input class="submit-get form-check-input" id="PHOIHOPXL_${
					item.users.id
				}" type="radio" data-id="${
					item.users.id
				}" name="srole[${
					item.users.id
				}]" onclick="checkClearCheckboxSrole('${
					item.users.id
				}')" value="PHOIHOPXL" />
                    <label class="form-check-label m-0" for="PHOIHOPXL_${
					item.users.id
				}">${PHOIHOPXL}</label>
                  </div>
              </div>
            ...
            <td class="align-middle">
              <div class="form-group">
                  <div id="datepicker-container_${
					item.users.id
				}" class="datepicker-container" > 
                      <span style="position: relative;" class="outline-element-container"> 
                      <input id="dt_issued_date_dealine_${
					item.users.id
				}" type="text" name="deadline[${
					item.users.id
				}]" data-user-dealine="${
					item.users.id
				}" value="${Time_Now}" class="form-control openemr-datepicker input-textbox outline-element incorrect" objtype="7" maxlength="10" name="action_element" aria-label="Select Date"> 
                      <span class="correct-incorrect-icon"> </span>
                      </span>
                      <div id="datepicker" ></div>
                    </div>
            ...
            <td class="p-0 align-middle">
              <textarea id="content_process" class="form-control" name="content[${
					item.users.id
				}]"></textarea>
            </td>
            <td class="p-0 align-middle" style="text-align: center;">
              <span class="fas fa-trash text-danger" style="cursor: pointer;" onclick="removeUserChosses(${
					item.users.id
				})"></span>
            </td>
        </tr>
...
formMediaDepartmentVT.addEventListener("upload_success", (data) => {
	...
	$("#form_assign_VT").append(`<input name="media_ids[]" value = "${
		data.id
...
formMediaLeaderHandle.addEventListener("upload_success", (data) => { // đưa id media vào input media_ids của form mandocfile
	formMediaPreview.removeTableItemAll();
	mediaFileRelease.removeTableItemAll();
	...
	formMediaAssignLeader.removeTableItemAll();
	formMediaMandocfileUser.removeTableItemAll();
	$("#new_mandocdhandle").append(`<input name="media_ids[]" value = "${
		data.id
	}" style="display: none"></input>`)
});
...
formMediaMandocfile.addEventListener("upload_success", (data) => {
	...
	formMediaMandocfileUser.removeTableItemAll();
	// đưa id media vào input media_ids của form mandocfile
	$("#form_add_document_infor").append(`<input name="media_ids[]" value = "${
		data.id
	}" style="display: none"></input>`)
...
mediaFileRelease.addEventListener("upload_success", (data) => {
	...
	formMediaMandocfileUser.removeTableItemAll();
	// đưa id media vào input media_ids của form mandocfile
	$("#form_release_mandoc").append(`<input name="media_ids[]" value = "${
		data.id
	}" style="display: none"></input>`)
});
...
formMediaMandocfileUser.addEventListener("upload_success", (data) => {
	...
	formMediaAssignLeader.removeTableItemAll();
	// đưa id media vào input media_ids của form mandocfile
	$("#upload_file_mandocfile_user").append(`<input name="media_ids[]" value = "${
		data.id
	}" style="display: none"></input>`)
});
...
formMediaAssignLeader.addEventListener("upload_success", (data) => {
	...
	formMediaMandocfileUser.removeTableItemAll();
	// đưa id media vào input media_ids của form mandocfile
	$("#assign_leader_BLD").append(`<input name="media_ids[]" value = "${
		data.id
	}" style="display: none"></input>`)
});
...
formMediaAssignDepratment.addEventListener("upload_success", (data) => {
	...
	formMediaMandocfileUser.removeTableItemAll();
	// đưa id media vào input media_ids của form mandocfile
	$("#form_add_mandoc_dhandle").append(`<input name="media_ids[]" value = "${
		data.id
	}" style="display: none"></input>`)
});
	...
	if (datas.length > 0) {
		...
		datas.forEach(function (item, index) {
			var newOption = new Option(`${
				...
				item.positionjob == null ? "" : item.positionjob.name
			} - ${
				item.department == null ? "" : item.department.name
			}]`, item.users.id, false, false);
			$('#select_release_staff').append(newOption).trigger('change');
		});
	}
...
// Call department when #MODAL-VBD-BLD-ASSIGN-PB show
$('#MODAL-VBD-BLD-ASSIGN-PB').on('shown.bs.modal', function () {
	var depId = $('.form_VT_XL select#department_id').val();
	if (!depId) return;
	$.ajax({
		...
		success: function (data) {
			...
			$.each(data, function (_, user) {
				var selected = user.positionjob_name.includes("Trưởng") ? ' selected' : '';
				usersArr.push('<option value="' + user.id + '"' + selected + '>' +
					user.last_name + ' ' + user.first_name + ' (' + user.positionjob_name + ')' +
					'</option>');
			});
...
// Call department when #MODAL-EDIT-VBD show
$('#MODAL-EDIT-VBD').on('shown.bs.modal', function () {
	var depId = $('#form_add_document_infor select#mdepartment').val();
	if (!depId) return;
	$.ajax({
		url: mandocs_get_department_path,
		method: 'get',
		data: { department_id: depId },
		success: function (data) {
			var usersArr = [], department = "";
			$.each(data, function (_, user) {
				var selected = user.positionjob_name.includes("Trưởng") ? ' selected' : '';
				usersArr.push('<option value="' + user.email + '"' + selected + '>' +
					user.last_name + ' ' + user.first_name + ' (' + user.positionjob_name + ')' +
					'</option>');
...
// Call department when department_id change
...
// Call department when select#mdepartment change
...
function initTinymce() {
	var tinymceOptions = {
		language_url: '/assets/myjs/vi.js',
		language: 'vi',
		...
		templates: [
			{
				title: 'Mẫu nội dung ban hành văn bản của BMTU',
				description: 'Xem trước mẫu - "Mẫu nội dung ban hành văn bản của BMTU"',
        ...
        <div dir="ltr"><span style="font-family: arial, helvetica, sans-serif; font-size: 12pt;"><strong><em><span style="color: #0000ff;"><img src="https://erp.bmtu.edu.vn/assets/logo-ee2ae1c651c861edbaacdaaf34c656a8ce869f36ff819de7ef86df69b9c7d3e1.png" width="238" height="87"></span></em></strong></span></div>
...
function onOpenUserHandleForm(mandocId, uHandleId, uHandleContents, assignName) { // form static data
...
function open_form_assign_departments(mandocid, content, sfrom, received_at, effective_date, iduhandle, mdepartment, deadline) { // choices_department.removeActiveItems();
	$("#contents").val("");
...
function get_mandoc_info(mandoc_id, content, received_at, effective_date, uhandle_id, department_id, deadline, dhandle_id) { // call remote form to get mandoc content
...
function clickLeaderHandle(mandoc_id, content, received_at, effective_date, uhandle_id, department_id, deadline, dhandle_id, last_uhandle_contents, sothers, showUnapproval) {
	var elementsToEnable = ["#modal-vbd-bld-assgin-vt", "#modal-vbd-bld-assgin-pb",];
	var  elementsToDisable = ["#modal-vbd-form-assign-vt", "#modal-vbd-form-assign-pb",];
	if (sothers == "DUYETVANBAN") {
		elementsToEnable.forEach(function (elementId) {
			toggleElementState(elementId, true);
		});
	} else {
		elementsToDisable.forEach(function (elementId) {
			toggleElementState(elementId, false);
		});
	}
...
function openFormAssign(element) {
	...
	$("#btn_handle_vbd_" + mandoc_id).click();
}
...
function loadReleaseMandocFormData(mandoc, files) {
	...
	if (mandoc) {
		if (mediaFileRelease) {
			...
			// formMediaAssignLeader.removeTableItemAll();
			// formMediaMandocfileUser.removeTableItemAll();
		} else { // TODO: debug issue to fix this
		}
		$("#modal-release-mandoc").find(".choice-wrap").show();
		$("#modal-release-mandoc").find("#form_release_mandoc").hide();
		$("#modal-release-mandoc").find("#form_save_release_mandoc").hide();
		$("#modal-release-mandoc").modal("show");
	} else { // TODO: show not found message?
	}
}
...
$("#user_vt_handle").click(function () {
	...
	for (let i = 0; i < input_radio.length; i++) {
		...
		$("#assign_leader_BLD").append(`<input name="option_media[]" value="${
			input_radio[i].value
		}" style="display: none">`);
	}
...
function onclickSubmitVTPB() {
	...
	for (let i = 0; i < input_radio.length; i++) {
		...
		$("#form_add_mandoc_dhandle").append(`<input name="option_media[]" value="${
			input_radio[i].value
		}" style="display: none">`);
	}
...
$("#vt_ban_hanh").click(function () {
	...
	for (let i = 0; i < input_radio.length; i++) {
		...
		$("#form_release_mandoc").append(`<input name="option_media[]" value="${
			input_radio[i].value
		}" style="display: none">`);
	}
...
$("#save_no_release").click(function () {
	...
	for (let i = 0; i < input_radio.length; i++) {
		...
		$("#form_save_release_mandoc").append(`<input name="option_media[]" value="${
			input_radio[i].value
		}" style="display: none">`);
	}
...
function conFirmHandle() {
	...
	for (let i = 0; i < input_radio.length; i++) {
		...
		$("#form-user-process").append(`<input name="option_media[]" value="${
			input_radio[i].value
		}" style="display: none">`);
	}
	var get_name = $("#assign_name").val();
...
// preview mandoc
const foemMediaDepartment = new FormMedia("media-assign-department-bld");
...
function onHandleVBDBLD(mandoc_id, content, received_at, effective_date, uhandle_id, department_id, deadline, dhandle_id, sothers, path) {
	var elementsToDisable = ["#modal-vbd-bld-assgin-vt", "#modal-vbd-bld-assgin-pb",];
	var elementsToEnable = [".modal-vbd-form-assign-vt", ".modal-vbd-form-assign-pb",];
	if (sothers == "THU_KY_HIEU_TRUONG") {
		elementsToEnable.forEach(function (elementId) {
			toggleElementState(elementId, true);
		});
	} else {
	...
	showLoadding(true);
	callGetMandocRemote(mandoc_id, "loadPBViewData");
	callGetMandocRemote(mandoc_id, "loadLeaderViewData");
	foemMediaDepartment.removeTableItemAll();
	formMediaLeaderView.removeTableItemAll();

	// choices_department.removeActiveItems();
	$("#MODAL-VBD-BLD-FORM-ASSIGN-VT #contents").val("");
	$("#MODAL-VBD-BLD-FORM-ASSIGN-VT #iduhandle").val(uhandle_id);
	$("#MODAL-VBD-BLD-FORM-ASSIGN-VT #mandoc_id_dhandle").val(mandoc_id);

	content = decodeURIComponent(content).replace(/\+/g, ' ');
	$("#MODAL-VBD-BLD-FORM-ASSIGN-VT textarea#mandoc_dhandle_Proposal_of_the_TC-HC").val(content);
	$("#MODAL-VBD-BLD-FORM-ASSIGN-VT textarea#mandoc_dhandle_Proposal_of_the_TC-HC").prop("readonly", true);

	if (effective_date == "") {
		$("#MODAL-VBD-BLD-FORM-ASSIGN-VT #deadline\\ dis_enter_out").val(received_at);
	} else {
		$("#MODAL-VBD-BLD-FORM-ASSIGN-VT #deadline\\ dis_enter_out").val(effective_date);
	}

	// choices_department.removeActiveItems();

	dcontent = decodeURIComponent(content).replace(/\+/g, ' ');
	$("#MODAL-VBD-BLD-ASSIGN-PB textarea#contents").val(dcontent);
	...
	reloadChoicesVBD();
	$("#form_asign_mandoc_department_vbd #iduhandle_pb_handle_user").val(uhandle_id);
	$("#form_asign_mandoc_department_vbd #mandoc_id_handle_user").val(mandoc_id);
	$("#form_asign_mandoc_department_vbd #content_mandoc_department_process").val(content);
	if (effective_date == "") {
		$("#form_asign_mandoc_department_vbd #deadline").val(received_at);
	} else {
		$("#form_asign_mandoc_department_vbd #deadline").val(effective_date);
	}
	$("#MODAL-VBD-BLD-ASSIGN-PB form#form_asign_mandoc_department_vbd").attr('action', path)
	$("#modal-handle-van-ban-di").modal('show');
}

function onHandleVBT(mandoc_id, content, received_at, effective_date, uhandle_id, department_id, deadline, dhandle_id, sothers) {
	var elementsToDisable = ["#modal-handle-van-ban-toi #modal-vbt-vt-assgin-tchc"];
	var elementsToEnable = ["#modal-handle-van-ban-toi #modal-form-assign-bld"];
	if (sothers == "VTTIEPNHAN") {
		elementsToEnable.forEach(function (elementId) {
			toggleElementState(elementId, true);
		});
	} else {
	...
	$("#modal-handle-van-ban-toi").modal('show');
}
...
function onHandleVBTBGH(mandoc_id, content, received_at, effective_date, uhandle_id, department_id, deadline, dhandle_id, sothers, mdepartment) {
	var elementsToDisable = ["#modal-handle-van-ban-toi #modal-vbt-vt-assgin-tchc"];
	var elementsToEnable = ["#modal-handle-van-ban-toi #modal-form-assign-bld"];
	if (sothers == "VTTIEPNHAN") {
		elementsToEnable.forEach(function (elementId) {
	...
	$("#iddhandle").val(dhandle_id);
	VT_contents = decodeURIComponent(content).replace(/\+/g, ' ');
...
function onHandleVBTAPPROVE(mandoc_id, dcontent, received_at, effective_date, uhandle_id, department_id, deadline, dhandle_id, sothers, mdepartment, content,dhandle_last) {
	showLoadding(true);
    ...
    dcontents = decodeURIComponent(dcontent).replace(/\+/g, ' ');
    $("textarea#content_department_process").val(dcontent);
    $("textarea#content_department_process").prop("readonly", true);
    document.getElementById("mandoc_content_user_process").value  = content;
    document.getElementById("iduhandle_pb_handle_user_process").value = uhandle_id;
    document.getElementById("id_department_user_process").value = dhandle_last;
	$("#check_mandocs_inactive").modal('show');
}
function onHandleVBTASSINGPB(mandoc_id, dcontent, received_at, effective_date, uhandle_id, department_id, deadline, dhandle_id, sothers, mdepartment, content,dhandle_last,path) {
	showLoadding(true);
	callGetMandocRemote(mandoc_id, "loadPBViewData");
	callGetMandocRemote(mandoc_id, "loadLeaderViewData");
	foemMediaDepartment.removeTableItemAll();
	...
	$("#MODAL-VBD-BLD-ASSIGN-PB form#form_asign_mandoc_department_vbd").attr('action', path)
	$("#MODAL-VBD-BLD-ASSIGN-PB").modal('show');
}
function onHandleVBTTCHC(mandoc_id, content, received_at, effective_date, uhandle_id, department_id, deadline, dhandle_id, sothers, mdepartment) {
	var elementsToDisable = ["#modal-handle-van-ban-toi #modal-vbt-vt-assgin-tchc"];
	var elementsToEnable = ["#modal-handle-van-ban-toi #modal-form-assign-bld"];
	if (sothers == "VTTIEPNHAN") {
		elementsToEnable.forEach(function (elementId) {
...
function toggleElementState(elementId, state) {
	$(elementId).toggleClass('d-none', state);
	$(elementId).prop('disabled', state);
}
function onHandleVBD(mandoc_id, content, received_at, effective_date, uhandle_id, department_id, deadline, dhandle_id, sothers) {
	var elementsToDisable = ["#modal-handle-van-ban-di #modal-vbd-bld-assgin-vt", "#modal-handle-van-ban-di #modal-vbd-bld-assgin-pb",];
	var elementsToEnable = ["#modal-handle-van-ban-di #modal-vbd-form-assign-vt", "#modal-handle-van-ban-di #modal-vbd-form-assign-pb",];
	if (sothers == "THU_KY_HIEU_TRUONG") {
		elementsToEnable.forEach(function (elementId) {
			toggleElementState(elementId, false);
		});
...
function loadLeaderMedias(mandoc,files){
	...
	//   formMediaManfile.removeTableItemAll();
	//   formMediaMandocfileUser.removeTableItemAll();
	  $('#MODAL-VBT-APPROVE-MANDOC-LEADER-DEPARTMENT').modal("show");
...
function openLeaderHandle(mandoc_id, content, received_at, effective_date, uhandle_id, department_id, deadline, dhandle_id, last_uhandle_contents,userIdNearEnd, showUnapproval){
	...
	// form data
	uhandle_last_contents = decodeURIComponent(last_uhandle_contents).replace(/\+/g, ' ');
	$("#MODAL-VBT-APPROVE-MANDOC-LEADER-DEPARTMENT textarea#contents_handle_user").val(uhandle_last_contents);
	$("#MODAL-VBT-APPROVE-MANDOC-LEADER-DEPARTMENT textarea#contents_handle_user").prop("readonly", true);
	let btnUnapproval = $("#MODAL-VBT-APPROVE-MANDOC-LEADER-DEPARTMENT .btn-unapproval");
	// if(showUnapproval){
	  btnUnapproval.show();
	  btnUnapproval.attr("data-id",mandoc_id);
	  btnUnapproval.attr("data-content",last_uhandle_contents);
	  btnUnapproval.attr("data-id-dhandle",dhandle_id);
	  btnUnapproval.attr("data-deadline",deadline);
	// }else{
	//   btnUnapproval.hide();
