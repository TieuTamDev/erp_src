(function($) {
  $.fn.resizableColumns = function() {
    ...
    _table.find('.resizer').mousedown(function(e) {
      ...
      thead_resize = e.currentTarget.parentElement.parentElement.parentElement;
      $(thead_resize).find('th').removeClass('resizing');
        $(thead_resize).find('tr:first-child th:nth-child(' + ($(this).closest('th').index() + 1) + ') .resizer').closest('th').addClass('resizing');
    ...
    _table.mousemove(function(e) {
        if (isColResizing) {
          var current_head =  $(thead_resize);
          
          var _resizing = current_head.find('th.resizing .resizer');
          
          if (_resizing.length >= 1) {
            var _nextRow = current_head.find('th.resizing + th');
